...
docker run --name dbtest-postgres -p 5432:5432 -e POSTGRES_PASSWORD=6c9FT%Kj -e POSTGRES_USER=travis -d postgres:latest
...
# docker exec -it -e PGPASSWORD=r2N5y7V -e POSTGRES_USER=travis dbtest-postgres psql -U travis -c "CREATE USER IF NOT EXISTS travis@'%' IDENTIFIED BY '6c9FT%Kj'; GRANT ALL ON *.* TO travis@'%'; FLUSH PRIVILEGES;"
#
